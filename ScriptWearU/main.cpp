...
void check_game(int rL); //checks if the game is a game that Script-Wear can do game specific scripts on

LONG roblox_wnd_proc;
HWND roblox_window;
void handle_command(const std::string& command); //handles command input via Script-Wear's command bar
...
void tutorial() {
	MessageBox(NULL, "Welcome to the Script-Wear tutorial! Due to this being your first time using Script-Wear, you have been granted with an easy tutorial sequence of messageboxes. These are designed to help with how to use Script-Wear.\nIf you would like to re-play this, just execute in the command box; 'tutorial' and it will replay.", "Script-Wear", MB_OK);
	MessageBox(NULL, "You're able to automatically execute any script you want straight when Script-Wear has completed its loading. Just go to your Script-Wear folder, go into the data folder and then into the autoexec folder, place any .lua or .txt file you want to be auto ran inside. You can have as many as you would like!\n", "Script-Wear", MB_OK);
	MessageBox(NULL, "ScriptWear's main user interface consists of a main script box, an output box and a small strip under the output box, this box is called the command bar/box", "Script-Wear", MB_OK);
	MessageBox(NULL, "In this bar you can execute commands that enhance your user experience with Script-Wear, why dont you go ahead and run 'cmds' in that box right away!", "Script-Wear", MB_OK);
	MessageBox(NULL, "This command bar also contains several pre-built scripts, these scripts can be ran by running the 'run' command, a space and then the preset script from the short list you are given.\n(for example, lets say i wanted to run the rocky2u script, i would put in the command bar 'run rocky2u' and hit enter)", "Script-Wear", MB_OK);
	MessageBox(NULL, "After you're done running commands in the command bar, make sure to run the 'cls' command! This command will clear out the output box, making it neat and tidy; just as you recieved it!", "Script-Wear", MB_OK);
	MessageBox(NULL, "You are able to easily run scripts by inputting them into the big box, which is called the script box, once the script has been inserted you are free to hit the execute button and run the script.\nIf you would like to find a script, you can click the Open button and look through your files for a txt or lua file which once double clicked will automatically be sent to the script box.", "Script-Wear", MB_OK);
	MessageBox(NULL, "If you would like to close Script-Wear, you can either hit the big X and simply close the entire thing alongside Roblox, or you could go inside the Script-Wear tab (thing at the top of the UI that says ScriptWear) and select de-attach, this will remove Script-Wear but keep Roblox open", "Script-Wear", MB_OK);
	MessageBox(NULL, "Fun Fact: Did you know Script-Wear has a cool chat system!? If you run 'chat' in the command bar, you have an activated chat script which allows you to chat with other users using Script-Wear in your server!", "Script-Wear", MB_OK);
	MessageBox(NULL, "Fun Fact #2: You can run 'load' alongside any script you currently have inside data->scripts and it will directly load the script into the script box! This is super useful for simple adjustments and whatnot.", "Script-Wear", MB_OK);
	MessageBox(NULL, "If you have questions, do not hesitate to contact AzuLX#7384 via Discord!\nThank you and enjoy! Don't forget you can replay this anytime! Just run 'tutorial' in the command bar.", "Script-Wear", MB_OK);
}
...
const char* commands = {
	...
	"chat - Runs a pre-made chat roblox gui that allows you to speak with members who are using Script-Wear and are in your game. Must be ran with 'run' just as the above command\r\n"
	...
	"tutorial - Re-plays the tutorial (sequence of message-boxes) you are provided upon first-time launch of Script-Wear\r\n"
...
void handle_command(const std::string& command) {
	...
	if (arguments.at(0) == "cmds" || arguments.at(0) == "commands") {
		MessageBox(NULL, commands, "Script-Wear", MB_OK);
	}
		...
		if (arguments.at(1) == "chat") {
			std::string chatsscript = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/chatgui.lua");
			execute_script(rL, L, chatsscript);
			return;
		}

		if (arguments.at(1) == "rocky" || arguments.at(1) == "rocky2u") {
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/rocky2u.lua");
		...
		{
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/iorb.txt");
			execute_script(rL, L, script);
			return;
		}
		...
		if (arguments.at(1) == "dex") {
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/dex.lua");
			execute_script(rL, L, script);
			return;
		}

		if (arguments.at(1) == "noobbat" || arguments.at(1) == "bat") {
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/noobbat");
			execute_script(rL, L, script);
			return;
		}

		if (arguments.at(1) == "zanoob" || arguments.at(1) == "noob") {
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/zanoob.lua");
			execute_script(rL, L, script);
			return;
		}

		if (arguments.at(1) == "pyro") {
			std::string script = download_url("https://raw.githubusercontent.com/azulx/ScriptWear/master/scripts/pyro.lua");
			execute_script(rL, L, script);
			return;
		}
		...
		print_output(RGB(0, 0, 0), "Script-Wear v%s Loaded! ", SW_VERSION);
...
static LRESULT CALLBACK dll_window_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_COMMAND:
	{
		...
		case BUTTON_CLEAR_SCRIPTEDITOR:
		{
			...
			SendMessage(script_field, SCI_SETSCROLLWIDTH, 1, 1);
			break;
		}
		...
		case BUTTON_REMEMBERME:
		{
			if (!loadedscr)
			{
				MessageBox(NULL, "You're unable to store your data due to an error with the setting of files. Please re-launch Script-Wear and then attempt to re-do this", "Script-Wear", MB_ICONERROR | MB_OK);
				break;
			}
			...
			std::string encoded_password = password_buffer;
		...
		case MENU_EXIT: {
			if (MessageBox(NULL, "Are you sure you want to close Script-Wear? Roblox will close with it.", "Confirmation", MB_ICONWARNING | MB_YESNO) == IDYES) {
				exit(0);
			}
			break;
		}
		case MENU_DEATTACH:
		{
			if (MessageBox(NULL, "Are you sure you want to close Script-Wear? This will close Script-Wear but not Roblox", "Confirmation", MB_ICONWARNING | MB_YESNO) == IDYES) {
				ExitThread(0);
			}
			break;
		}
		case MENU_ABOUT:
		{
			MessageBox(NULL, "Script-Wear | The unrestricted script execution product\n\nWritten and developed by AzuLX and Quad Core", "Script-Wear", MB_ICONINFORMATION | MB_OK);
			break;
		}
		default:
...
static BOOL register_window_class(const char* wclassname) {
	...
	nclass.hbrBackground = (HBRUSH)(CreateSolidBrush(RGB(0x21, 0x21, 0x21)));
	nclass.lpszMenuName = "ScriptWear";
...
{
	HMENU ScriptWear_menu = CreatePopupMenu();

	AppendMenu(hhmenu, MF_POPUP, (UINT_PTR)ScriptWear_menu, TEXT("Script-Wear"));
	AppendMenu(ScriptWear_menu, MF_STRING, MENU_EXIT, TEXT("Exit"));
	AppendMenu(ScriptWear_menu, MF_STRING, MENU_DEATTACH, TEXT("De-Attach"));
	InsertMenu(ScriptWear_menu, 2, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	AppendMenu(ScriptWear_menu, MF_STRING, MENU_ABOUT, TEXT("About"));
}

HMENU create_windowmenu()
{
	HMENU ScriptWear_menu = CreateMenu();

	if (ScriptWear_menu == NULL)
		return FALSE;

	create_toolbar_menu(ScriptWear_menu);
	return ScriptWear_menu;
}
...
void remember_me_functionality()
{
	...
	if (swfile1.is_open())
	{
		...
		{
			SendMessage(password_field, WM_SETTEXT, (WPARAM)"", (LPARAM)line1.c_str());
...
static void init_gui() {
	...
	if (is_lite)
	{
		sw_title = "Script-Wear | Lite Edition";
	}

	else if (is_extended)
	{
		sw_title = "Script-Wear | Extended Edition";
	}

	else
	{
		sw_title = "Script-Wear";
	}

	HINSTANCE hinstance = GetModuleHandle(0);

	UnregisterClass("ScriptWear", hinstance);

	register_window_class("ScriptWear");
	register_window_class("ScriptWearLoading");
	register_window_class("ScriptWearLogin");
	...
	/* Login Screen */
	main_login_window = CreateWindowEx(0, "ScriptWearLogin", "Script-Wear", WS_SYSMENU | WS_MINIMIZEBOX, 100, 100, 316, 293, 0, 0, hinstance, 0);
	...
	HWND label1 = CreateWindow(TEXT("static"), "Script-Wear - Developed by AzuLX and Quad Core", WS_CHILD | WS_VISIBLE | WS_TABSTOP, 10, 240, 300, 190, main_login_window, (HMENU)(501), hinstance, NULL);
	...
	/* Loading Screen */
	main_load_window = CreateWindowEx(0, "ScriptWearLoading", "Script-Wear", WS_SYSMENU, 100, 100, 316, 120, 0, 0, hinstance, 0);
	...
	/* Main Screen */
	main_window = CreateWindowEx(0, "ScriptWear", sw_title.c_str(), WS_SYSMENU | WS_MINIMIZEBOX, 100, 100, 407, 380, 0, window_menu, hinstance, 0);
	...
	execute = CreateWindowEx(0, "BUTTON", "Execute", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 11, 175, 119, 33, main_window, (HMENU)BUTTON_EXECUTE_SCRIPT, hinstance, 0);
	clear = CreateWindowEx(0, "BUTTON", "Clear", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 265, 175, 115, 33, main_window, (HMENU)BUTTON_CLEAR_SCRIPTEDITOR, hinstance, 0);
	open = CreateWindowEx(0, "BUTTON", "Open", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 136, 175, 123, 33, main_window, (HMENU)BUTTON_OPEN_SCRIPT, hinstance, 0);
	cmd_input_field = CreateWindowEx(0, RICHEDIT_CLASS, "", WS_CHILD | ES_READONLY | WS_VISIBLE | WS_HSCROLL | ES_MULTILINE | WS_BORDER, 11, 290, 369, 20, main_window, (HMENU)TXT_CMDINPUT, hinstance, 0);
	...
	SendMessage(script_field, SCI_STYLESETBACK, 32, (LPARAM)RGB(0x1C, 0x1C, 0x1C));  // set back-color of window

	SendMessage(script_field, SCI_STYLESETBACK, 33, (LPARAM)RGB(0x1C, 0x1C, 0x1C));  // set back-color of margin

	SendMessage(script_field, SCI_SETSCROLLWIDTH, 1, 1);
	SendMessage(script_field, SCI_SETSCROLLWIDTHTRACKING, true, true);
	...
	SendMessage(cmd_input_field, EM_SETBKGNDCOLOR, 0, (LPARAM)RGB(0x1C, 0x1C, 0x1C));
	SendMessage(output_field, EM_SETBKGNDCOLOR, 0, (LPARAM)RGB(0x1C, 0x1C, 0x1C));
	...
	remove_style(cmd_input_field, WS_EX_CLIENTEDGE);
...
static int teleport_handler(int rL) {
	//todo: make Script-Wear handle teleports to different servers by re-scanning
...
static int custom_getobjects(int rL) {
	if (is_lite)
	{
		print_output(RGB(255, 0, 0), "ERROR: game:GetObjects(asset) is not available on Script-Wear Lite!");
		return 1;
	}
...
void initialize() {
	...
	is_extended = true; // bro epic gamer mode :sunglasses:
	...
	output_label("ScriptWear: Loading...");

	L = luaL_newstate(); //initialize the vanilla lua state

	if (!L) {
		ERRORMSGBOX("Script-Wear", "Failed to initialize the lua state.");
		exit(1);
	}

	if (!is_wow64()) {
		output_label("ScriptWear: 32-bit system detected!");
		ERRORMSGBOX("Script-Wear currently does not support 32-bit systems. Please click OK to exit.", "Script-Wear");
		exit(0);
	}

	if (debug_mode)
		output_label("ScriptWear: Creating hooks");
	main();

	// Sleep(300);

	if (debug_mode)
		output_label("ScriptWear: Creating custom implementations to Script-Wear's environment");
	...
	if (debug_mode)
		output_label("ScriptWear: Executing scripts held inside data->autoexec");

	if (debug_mode)
		output_label("ScriptWear: Init script loading");
	...
	print_output(RGB(255, 255, 255), "Script-Wear v%s Loaded!\n", SW_VERSION);
	print_output(RGB(255, 255, 255), "Luau Edition (MADE BY ");
	print_output(RGB(20, 20, 255), "KowalskiFX#2382");
	print_output(RGB(255, 255, 255), " & ");
	print_output(RGB(20, 20, 255), "Ice Bear#3079");
	print_output(RGB(255, 255, 255), ")\n");

	if (auth_firsttime) {
		if (MessageBox(NULL, "Would you like to view the Script-Wear tutorial documentation?", "Script-Wear", MB_ICONINFORMATION | MB_YESNO) == IDYES) {
			tutorial();
			print_output(RGB(0, 0, 0), "Tutorial concluded. Welcome to Script-Wear!");
		}
	}
...
unsigned char* dwFindPattern(unsigned char* pData, unsigned int length, const unsigned char* pat, const char* msk)
{
...
DWORD WINAPI main() {
	...
	if (debug_mode)
		begin_console((PCHAR)"Script-Wear Debug Console");

	try {
		script_context = initializeWSJ(L);

		if (debug_mode)
			output_label("ScriptWear: Initializing lua state");


		rL = (script_context + LocalScriptGlobalState) - *(uintptr_t*)(script_context + LocalScriptGlobalState);
		...
		if (debug_mode)
			output_label("ScriptWear: Declaring ContentProvider");
		...
		MessageBox(NULL, "Exception handled during main scan. Please try again.", "Script-Wear", MB_ICONERROR | MB_OK);
		exit(0);
	}

	if (debug_mode)
		output_label("ScriptWear: Concluded Scan");
